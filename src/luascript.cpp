...
void LuaScriptInterface::registerFunctions()
{
	...
	registerEnum(ITEM_ATTRIBUTE_DODGE)
	...
	registerMethod("Player", "resetSkill", LuaScriptInterface::luaPlayerResetSkill);
	...
	registerMethod("ItemType", "getDodge", LuaScriptInterface::luaItemTypeGetDodge);
...
int LuaScriptInterface::luaPlayerResetSkill(lua_State* L)
{
	// player:resetSkill(skillType)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		skills_t skillType = getNumber<skills_t>(L, 2);
		player->resetSkill(skillType);
...
{
	// player:setSkill(skillType, value)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		skills_t skillType = getNumber<skills_t>(L, 2);
...
int LuaScriptInterface::luaItemTypeGetDodge(lua_State* L)
{
    // itemType:getDodge()
    const ItemType* itemType = getUserdata<const ItemType>(L, -1);
    if (itemType) {
        lua_pushnumber(L, itemType->dodge);
    } else {
        lua_pushnil(L);
    }
    return 1;
