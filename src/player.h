...
class Player final : public Creature, public Cylinder
{
	public:
		...
		void resetSkill(skills_t skill);
		void setSkill(skills_t skill, uint16_t value);

		int32_t getArmor() const final;
		int32_t getDodge() const;
	...
	protected:
		...
		uint32_t getAttackSpeed() const {
			uint32_t speed;
			speed = getVocation()->getAttackSpeed() - (getSkillLevel(SKILL_FIST) * 10);

			return speed > 500 ? speed : 500;
		}
