...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	local skill = player:getSoul()+50
	local loot1 = math.random(8, 18)
	local loot2 = math.random(0, 18)
		if (target.itemid == config.item1) and (skill >= rand) or (target.itemid == config.item1b) and (skill >= rand) then
				player:addItem(2225, loot1)
				...
				player:sendTextMessage(MESSAGE_INFO_DESCR, 'You got ' ..loot1.. ' pieces of iron.')
				end								
