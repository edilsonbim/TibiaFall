local noMoveStorage = 32321
local noMoveSeconds = 10
function Player.setNoMove(self, move, seconds)
	seconds = seconds and seconds or noMoveSeconds
	self:setStorageValue(noMoveStorage, move and (os.time() + seconds) or 0)
	if (move) then 
		playerStartNoMove(self:getId(), self:getPosition(), self:getDirection())
	end 
end 

function Player.getNoMove(self)
	local value = self:getStorageValue(noMoveStorage)
	return value and math.max(value, 0) or 0
end 

function Player.hasNoMove(self)
	return self:getNoMove() > os.time()
end 

function playerStartNoMove(cid, fromPosition, direction)
	local player = Player(cid)
	if (player) then
		if (player:hasNoMove()) then 
			player:teleportTo(fromPosition, true)
			player:setDirection(direction)
			addEvent(playerStartNoMove, 100, cid, fromPosition, direction)
		end
	end 
end 

function Player.removeHealthFromSpell(self, type, min, max, effect)
	doTargetCombatHealth(0, self, type, -min, -max, effect)
end 

function Player.getDodge(self)
	local dodge = 0
	...
	for i = 1, #slots do 
		...
		if (item) then 
			if (item:getType():getDodge() > 0) then 
				dodge = dodge + item:getType():getDodge()
			end 
		end
	end 
...
function Player:onLook(thing, position, distance)
	...
	if thing:isItem() then
		local charges = thing:getCharges()
		local chargesTotal = thing:getType():getCharges()
		if charges > 0 then
			local DUR = charges*100 / chargesTotal
			description = string.format("%s\nDur: [%0.1f]", description, DUR)
		end
	end
