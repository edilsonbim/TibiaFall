...
local function greetCallback(cid)
	...
	if level < 1 then
		...
		npcHandler:setMessage(MESSAGE_GREET, "Hello " .. player:getName() ..", do you want to join the {Elementalist} class?")
	end
	return true
end
...
local function creatureSayCallback(cid, type, msg)
	...
	if npcHandler.topic[cid] == 0 then
		if msgcontains(msg, "yes") then
			npcHandler:say("All right, now yous a Elementalist, don't dishonor your class!", cid)
			player:setVocation(14)
			player:getPosition():sendMagicEffect(CONST_ME_TELEPORT)
			player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You now is a Elementalist")
		end
	end
	return true
end
