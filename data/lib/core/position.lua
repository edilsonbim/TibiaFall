...
function testPositions(creature, position, direction, steps)
	if (steps > 0) then 
		local testPos
		local normalPos 
		local tile
		for i = 1, steps do 
			normalPos = position
			normalPos:getNextPosition(direction, i)
			tile = Tile(normalPos)
			normalPos:sendMagicEffect(CONST_ME_WATERSPLASH)
			if (tile) then 
				if (tile:queryAdd(creature) ~= RETURNVALUE_NOERROR) then 
					return false
				else
					creature:teleportTo(normalPos)
				end
			else
				return false
			end
